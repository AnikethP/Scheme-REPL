...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        pair = args
        while pair:
            arguments_list.append(pair.first)
            pair = pair.rest
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        env.define(target.first, lambda_proc)
        return target.first
...
def do_and_form(expressions, env):
    ...
    """
    if not expressions:
        return scheme_eval(True, env, True)
    while expressions.rest:
        if is_false_primitive(scheme_eval(expressions.first, env)):
            return scheme_eval(False, env, True)
        expressions = expressions.rest
    return scheme_eval(expressions.first, env) if is_true_primitive(expressions.first) else scheme_eval(False, env, True)
...
def do_or_form(expressions, env):
    ...
    while expressions.rest:
        ...
        expressions = expressions.rest
    return scheme_eval(False, env) if is_false_primitive(expressions.first) else scheme_eval(expressions.first, env, True)
...
def make_let_frame(bindings, env):
    ...
    names = values = nil
    while bindings:
        operands = bindings.first
        validate_form(operands, 2, 2)
        names = Pair(operands.first, names)
        values = Pair(scheme_eval(operands.rest.first, env), values)
...
def do_define_macro(expressions, env):
    ...
    validate_form(expressions.first, 0)
    
    macro_proc = MacroProcedure(expressions.first.rest, expressions.rest, env)
    env.define(expressions.first.first, macro_proc)
